...
1. 保留每天本地午夜主动轮转行为，并在 README 中明确说明。
2. 保持 `FileLogger`、`Sync` 和 `Close` 的 v1 API 签名，通过新增 API 暴露错误。
3. 禁止 `appName` 包含多级目录，目录层级统一由 `Config.Dir` 控制。
4. 路径校验处理不可信相对路径和调用时可见的符号链接；部署环境负责保证日志根目录不被不可信本机主体并发修改。
